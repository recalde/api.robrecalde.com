...
The SAM CLI installs dependencies defined in `src/recalde-api/recalde-api.csproj`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
Resources:
  ...
  NetCodeWebAPIServerless:
    ...
    Properties:
      CodeUri: ./src/recalde-api/
      Handler: recalde-api::recalde-api.Function::FunctionHandler
      Runtime: dotnet6
      MemorySize: 1042
      DeadLetterQueue:
        Type: SQS
...
```bash
recalde-api$ sam build
recalde-api$ sam deploy
```
...
```bash
{{ cookiecutter.project_name }}$ dotnet test test/recalde-api.Tests/recalde-api.Tests.csproj 
```
